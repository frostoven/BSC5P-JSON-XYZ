...
int main()
{
    ...
    for (t = 50; t <= 100000; t+= 1) {
        bbTemp = t;
        spectrum_to_xyz(bb_spectrum, &x, &y, &z);
        xyz_to_rgb(cs, x, y, z, &r, &g, &b);
        printf("  \"%1.0f\": {     \"x\": %.4f, \"y\": %.4f, \"z\": %.4f,   ", t, x, y, z);
        if (constrain_rgb(&r, &g, &b)) {
            norm_rgb(&r, &g, &b);
            printf(" \"r\": %.3f, \"g\": %.3f, \"b\": %.3f, \"note\": \"Approximation\"", r * multiplier, g * multiplier, b * multiplier);
        } else {
            norm_rgb(&r, &g, &b);
               printf(" \"r\": %.3f, \"g\": %.3f, \"b\": %.3f ", r * multiplier, g * multiplier, b * multiplier);
        }
        if (t != 100000) {
          printf(" },\n");
        }
